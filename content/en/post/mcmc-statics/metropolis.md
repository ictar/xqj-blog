...
2. **Acceptance** $\alpha_{ij}$: Decides "Do I really jump there, or stay put?".
   ...
   - ⚠️ The entire reason Metropolis algorithm exists is because the state space is too large (or continuous infinite), making it impossible to pre-determine this "2D array" of acceptance rates.
...
1. **Scatter Plot**
   ...
   * σ=0.05: Dense cloud, stuck locally.
   * σ=0.5: Cloud covers elliptical distribution, reasonable.
   * σ=2.0: Reasonable distribution, but "jumpy" trace, many rejections (trace gets stuck).

2. **ACF (x-dim)**

   * σ=0.05: Slow ACF decay → Strong correlation.
   * σ=0.5: Fast decay → High efficiency.
   * σ=2.0: Faster decay → Seemingly high efficiency, but low acceptance leads to stability issues.
...
* Too large → Low acceptance, chain "stuck".
